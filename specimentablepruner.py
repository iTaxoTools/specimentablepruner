...
def gui_main() -> None:
    ...
    if os.name == "nt":
        root.wm_iconbitmap(os.path.join(resource_path, 'data', 'specimentablepruner_icon.ico'))

    stype = ttk.Style()
    stype.configure(style="PruneButton.TButton", background="blue")

    banner_frame = ttk.Frame(root)
    logo_img = tk.PhotoImage(file=os.path.join(resource_path, 'data', 'iTaxoTools Digital linneaeus MICROLOGO.png'))
